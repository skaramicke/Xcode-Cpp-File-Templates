//
//  ___FILENAME___
//  ___PROJECTNAME___
//
//  Created by ___FULLUSERNAME___ on ___DATE___.
//___COPYRIGHT___
//

#ifndef _____VARIABLE_namespaceName________FILEBASENAMEASIDENTIFIER_____
#define _____VARIABLE_namespaceName________FILEBASENAMEASIDENTIFIER_____

class ___FILEBASENAMEASIDENTIFIER___ {
private:
#pragma mark - Members
    // members
    
public:
#pragma mark - Constructos
    ___FILEBASENAMEASIDENTIFIER___();
    ___FILEBASENAMEASIDENTIFIER___(const ___FILEBASENAMEASIDENTIFIER___& other) = default;
    ___FILEBASENAMEASIDENTIFIER___(___FILEBASENAMEASIDENTIFIER___&& other) = delete;    ~___FILEBASENAMEASIDENTIFIER___();
    
#pragma mark - Public methods
    // public methods
    
#pragma mark - Private methods
private:
    // private methods
    
public:
#pragma mark - Operators
    ___FILEBASENAMEASIDENTIFIER___& operator=(const ___FILEBASENAMEASIDENTIFIER___& other) = default;
    ___FILEBASENAMEASIDENTIFIER___& operator=(___FILEBASENAMEASIDENTIFIER___&& other) = delete;
};

#endif /* defined(_____VARIABLE_namespaceName________FILEBASENAMEASIDENTIFIER_____) */
